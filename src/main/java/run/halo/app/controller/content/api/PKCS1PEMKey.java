...
final class PKCS1PEMKey {
	...
	private static final Pattern PKCS1_PEM_KEY_PATTERN = Pattern
			.compile("(?m)(?s)^---*BEGIN RSA PRIVATE KEY.*---*$(.*)^---*END.*---*$.*");
	 ...
	 *
	 * <p>
	 * The Github app key comes in PKCS#1 format, while the Java security utilities
	 * only natively understand PKCS#8. Fortunately, we can convert between the two
	 * by adding the PKCS#8 headers manually.
	 *
	 * <p>
