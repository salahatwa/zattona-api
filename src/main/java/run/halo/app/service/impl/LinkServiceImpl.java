...
public class LinkServiceImpl extends AbstractCrudService<Link, Integer> implements LinkService {
    ...
    public @NotNull Link createBy(@NotNull LinkParam linkParam) {
        ...
        if (exist) {
            throw new AlreadyExistsException("Links " + linkParam.getName() + " 已存在").setErrorData(linkParam.getName());
        }
        ...
        if (exist) {
            throw new AlreadyExistsException("Links " + linkParam.getUrl() + " 已存在").setErrorData(linkParam.getUrl());
        }
