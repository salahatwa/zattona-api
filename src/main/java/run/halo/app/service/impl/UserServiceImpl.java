...
public class UserServiceImpl extends AbstractCrudService<User, Integer> implements UserService {
    ...
    public User updatePassword(String oldPassword, String newPassword, Integer userId) {
        ...
        if (oldPassword.equals(newPassword)) {
            throw new BadRequestException("The new password and the old password cannot be the same");
        }
        ...
        if (!BCrypt.checkpw(oldPassword, user.getPassword())) {
            throw new BadRequestException("Old password error").setErrorData(oldPassword);
        }
    ...
    public void mustNotExpire(User user) {
        ...
        if (user.getExpireTime() != null && user.getExpireTime().after(now)) {
            ...
            // If expired
            throw new ForbiddenException("The account has been disabled, please " + HaloUtils.timeFormat(seconds) + " 后重试").setErrorData(seconds);
        }
    }
    ...
    public User create(User user) {
        // Check user
        if (count() != 0) {
            throw new BadRequestException("Current blog has users");
        }
