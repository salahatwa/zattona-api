...
public class AdminAuthenticationFilter extends AbstractAuthenticationFilter {
    ...
    protected void doAuthenticate(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (StringUtils.isBlank(token)) {
            throw new AuthenticationException("Not logged in, please log in to access");
        }
        ...
        if (!optionalUserId.isPresent()) {
            throw new AuthenticationException("Token Expired or does not exist").setErrorData(token);
        }
